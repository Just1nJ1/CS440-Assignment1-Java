...
public class GridDisplay extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        matrix = new Button[height + 1][width + 1]; // allocates the size of the matrix
        ...
        for (int i = 0; i < height + 1; i++) {
            for (int j = 0; j < width + 1; j++) {
                matrix[i][j] = new Button(/*"(" + rand1 + ")"*/); //creates new random binary button
    ...
    public static void main(String[] args) {
        ...
        m = new Main(filename);
        System.out.println("Which Algorithm? (\"a\" for a star; \"h\" for h star)");
        String algo = sc.nextLine();
        List<Node> path = m.algo("a".equals(algo));
        if (path != null) {
